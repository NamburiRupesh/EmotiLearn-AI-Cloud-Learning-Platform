...
def signup():
    ...
    if not name or not email or not password or role not in ALLOWED_ROLES:
        return jsonify({"message": "Please provide a valid name, email, password and role."}), 400
    if len(password) < 8:
...
def login():
    ...
    if requested_role and requested_role not in ALLOWED_ROLES:
        return jsonify({"message": "Invalid portal selected."}), 400
    ...
    if requested_role and user["role"] != requested_role:
        return jsonify({"message": f"This account is registered as a {user['role']}. Please use the {user['role'].title()} portal."}), 403
